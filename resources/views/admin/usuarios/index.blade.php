            ...
            <x-adminlte-datatable id="table1" :heads="$heads">
                @foreach($users as $user)
                    <tr>
                        ...
                        <td>
                            {{-- @can('usuario.show') --}}
                                {{-- <a href="{{route('usuario.show',$user)}}" class="btn btn-xs btn-default text-teal mx-1 shadow" title="Mostrar">
                                    <i class="fa fa-lg fa-fw fa-eye"></i>
                                </a>                             --}}
                            {{-- @endcan --}}

                            {{-- @can('usuario.edit') --}}
                                <a href="{{route('usuario.edit',$user)}}" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar">
                                    <i class="fa fa-lg fa-fw fa-pen"></i>
                                </a>
                            {{-- @endcan --}}

                            {{-- @can('usuario.destroy') --}}
                                {{ Form::open(['route' => ['usuario.destroy', $user], 'method' => 'delete', 'class' => 'formEliminar', 'style' => 'display: inline']) }}
                                    @csrf
                                    @method('DELETE')
